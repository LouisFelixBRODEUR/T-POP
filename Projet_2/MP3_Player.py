...
My_simu = Microphone_Laser(sampling_rate_detector=1e5) # Base sampling_rate_detector = 5e6
...
# samples = samples/np.max(abs(samples))*1.4*My_simu.lambda_
...
samples = A_son* np.sin(2 * np.pi * F_son * time_raw)

# # Simulate sound a travers le setup
# start = time.time()
# t_sampled, I_avg = My_simu.simulate_detector_response(time_raw, samples)
...
_, I_raw = My_simu.simulate_raw_intensity(time_raw, samples)
t_sampled, I_avg =  My_simu.simulate_detector_response(time_raw, samples)
signal = My_simu.extract_sound_from_detector_response(I_avg)
...
# play_sound(time_raw, samples/np.max(samples))
# play_sound(t_sampled, signal/np.max(signal))



plt.figure(figsize=(10, 5))
plt.plot(time_raw, samples/np.max(samples), label="Raw Audio Signal (normalized)")
